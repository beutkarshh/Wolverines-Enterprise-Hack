...
class MultilingualVoiceEngine {
  ...
  // Returns raw MP3 Buffer — fast, per-sentence, used by streaming pipeline
  async streamVoiceToBuffer(text, language = 'en') {
    if (!this.voiceConfig[language]) language = 'en';
  ...
  // Pre-generate short filler audio files for instant playback while AI thinks
  async generateFillerAudio(outputDir = './public/audio') {
    const fillers = {
      en: 'Hmm, let me think...',
      hi: 'हाँ, एक moment...',
      mr: 'हो, थांबा...'
    ...
    for (const [lang, text] of Object.entries(fillers)) {
      const filePath = path.join(outputDir, `filler_${lang}.mp3`);
      if (!fs.existsSync(filePath)) {
        try {
          const buffer = await this.streamVoiceToBuffer(text, lang);
          fs.writeFileSync(filePath, buffer);
          console.log(`✅ Filler audio ready: filler_${lang}.mp3`);
        } catch (err) {
          console.warn(`⚠️  Filler generation skipped for ${lang}: ${err.message}`);
        }
      }
    }
  }
  ...
  /**
   * Convert any audio file to 8kHz mono 16-bit WAV required by Exotel Play verb.
   * Returns path to the new .wav file (saved alongside the source file).
   */
  async transcodeForPhone(inputPath) {
    const outputPath = inputPath.replace(/\.[^.]+$/, '_phone.wav');
    ...
    return new Promise((resolve, reject) => {
      ffmpeg(inputPath)
        ...
        .output(outputPath)
        .on('end', () => {
          console.log(`🔄 Transcoded for phone: ${path.basename(outputPath)}`);
          resolve(outputPath);
        })
        .on('error', (err) => {
  ...
  /**
   * Generate TTS specifically for phone calls via Exotel.
   * Returns { wavPath, webUrl } — wavPath is the 8kHz WAV file,
   * webUrl is the public-accessible URL Exotel can fetch.
   */
  async speakForPhone(text, language = 'en') {
    // Generate ElevenLabs MP3 to public/audio so it's web-accessible
    const result = await this.generateVoice(text, language, { webOutput: true, skipPlayback: true });
    if (!result.success) throw new Error('Voice generation failed for phone');

    // Transcode MP3 → 8kHz WAV in the same public/audio dir
    const wavPath = await this.transcodeForPhone(result.filePath);
    const wavFilename = path.basename(wavPath);
    const webUrl = `${process.env.BASE_URL || 'http://localhost:3001'}/audio/${wavFilename}`;

    console.log(`📞 Phone audio ready: ${webUrl}`);
    return { wavPath, webUrl, mp3Path: result.filePath };
  }
